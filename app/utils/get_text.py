...
def filter_bold_dates(bold_texts):
    ...
    month_names = r"(?:January|February|March|April|May|June|July|August|September|October|November|December)"  # noqa: E501
...
def get_text_in_sections(first_date, second_date, text: str):
    first_date_index, second_date_index = text.find(first_date), text.find(
        second_date
    )  # noqa: E501
...
while len(bold_lines) >= 2:
    ...
    pdf_data_dict["entries"][date[-4:]].append(
        {"date": date, "text": clean_text(section_text)}
    )
...
pdf_data_dict["entries"][date[-4:]].append(
    {
        "date": clean_text(bold_lines[0]),
        "text": clean_text(text[text.find(bold_lines[0]): -1]),
    }
)
